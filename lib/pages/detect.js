...
var Page = require('./page'),
    ...
    urls = require('../urls'),
    decodeEntities = require('htmldec'),
...
function processScriptArrays(doc, regex, keys, createObject) {
    var analyzer = Analyzer(doc),
        code = analyzer.getAllScriptsCode(),
        objs = [];

    // process all matches
    code.replace(regex, function (all, valuesStr) {
        var values = parseJSArray(valuesStr),
...
// parses a string containing a representation of a Javascript array containing primitive
// values and returns it
function parseJSArray(string) {
    var values = JSON.parse(escapeQuotes(string));
